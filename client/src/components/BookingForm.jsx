...
export default function BookingForm({ editingBooking, onEditDone, initialDate }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (hasConflict) {
      toast.error('Cannot submit — double booking conflict detected!');
      return;
    }
    ...
    <div className="bg-white rounded-2xl shadow-md overflow-hidden">
      ...
      {hasConflict && (
        <div className="mx-4 mt-4 bg-red-50 border border-red-300 rounded-lg p-3 flex items-start gap-2">
          ...
          <div>
            <p className="text-red-700 font-semibold text-sm">Double Booking Conflict!</p>
            {conflicts.map((c, i) => (
              <p key={i} className="text-red-600 text-xs mt-0.5">
                {c.roomUnit} is booked by <strong>{c.guestName}</strong> on this period.
              </p>
            ))}
          </div>
        </div>
